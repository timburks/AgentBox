              ...
              (then (set appfile-name ((versions 0) filename:))
                    ...
                    ((worker-count) times:
                     (do (i)
                         ...
                         (puts command)
                         (system command)
                         (set command (+ "chown -R agentbox " path "/var"))
                         (puts command)
                         (system command)
           ...
           (do (worker)
               (if (eq (uname) "Linux")
                   (then (set port (worker port:))
                         (set upstart-config-name (+ "/etc/init/agentbox-worker-" port ".conf"))
                         (system (+ "/sbin/initctl stop agentbox-worker-" port))
                         ((NSFileManager defaultManager) removeItemAtPath:upstart-config-name error:nil))
                   (else (set launchd-plist-name (+ "/Library/LaunchDaemons/net.agentbox.app."
                                                    (worker port:) ".plist"))
